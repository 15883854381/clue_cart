...
class Clue extends Model
{
    ...
    public function CluePhone($id, $type)
    {
        ...
        $sql = "SELECT a.clue_id,a.flag,a.provinceID,a.cityID,sales,Tosell,CONCAT(user_name,IF(sex = 1 ,'先生','女士')) as user_name , IF(sex = 1 ,'男','女') as sex ,
                 ...
                 IFNULL(IFNULL(notes_name,companyName),nickname) as clueName,h.total as upClueNum,h.allTotal,CEIL(((h.total/h.allTotal)*100)) as percentage 
                 FROM ${dataB} a 
    ...
    {
        $dataB = $type == 1 ? '`clue`' : 'clue_old';
        $sql = "SELECT a.clue_id,a.flag,a.provinceID,a.cityID,
                ...
                ROUND((100 / sales) * Tosell) as progress,d.`name` as brandname ,releaseNum,upClueNum,
                IFNULL(IFNULL(notes_name,companyName),nickname) as clueName,h.total as upClueNum,h.allTotal,CEIL(((h.total/h.allTotal)*100)) as percentage, 
                (CASE Tosell WHEN 0 THEN unitPrice_1  WHEN 1 THEN unitPrice_2 ELSE unitPrice_3 END) as Price
                FROM ${dataB} a 
